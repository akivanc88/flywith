        ...
        ScrollView {
            VStack(spacing: 20) {
                ...
                ZStack(alignment: .bottomLeading) {
                    LinearGradient(colors: [.indigo.opacity(0.8), .purple.opacity(0.6)], startPoint: .topLeading, endPoint: .bottomTrailing)
                        .frame(height: 200)
                    ...
                    VStack(spacing: 10) {
                        ...
                        HStack {
                            ...
                            Text("CAD \(Int(abs(recommendation.savings)))")
                                .fontWeight(.bold)
                                .foregroundStyle(recommendation.hasSavings ? .green : .indigo)
                        }
                        .font(.subheadline)
                    }
                }
                ...
                GroupBox("Visa info") {
                    HStack {
                        ...
                        Text("No visa required for Canadian passport holders. Free 30-day stay.")
                        Spacer()
                    }
                    .font(.subheadline)
                }
                ...
                // Book buttons
                VStack(spacing: 12) {
                    BookButton(title: "Book Leg 1 on Kiwi", subtitle: "\(recommendation.leg1.origin) → \(recommendation.leg1.destination) · CAD \(Int(recommendation.leg1.price))", url: recommendation.leg1.bookingURL, color: .indigo)
                    BookButton(title: "Book Leg 2 on Kiwi", subtitle: "\(recommendation.leg2.origin) → \(recommendation.leg2.destination) · CAD \(Int(recommendation.leg2.price))", url: recommendation.leg2.bookingURL, color: .green)
                    ...
                    .font(.subheadline).fontWeight(.semibold).foregroundStyle(.indigo)
                }
                .padding(.horizontal)
                .padding(.bottom, 32)
            }
...
struct ScoreRow: View {
    ...
    var body: some View {
        HStack {
            ...
            ProgressView(value: score / 5).tint(score >= 4 ? .green : score >= 3 ? .orange : .red)
